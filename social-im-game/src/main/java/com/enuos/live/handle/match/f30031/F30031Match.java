package com.enuos.live.handle.match.f30031;
...
@Component
@AbstractAction(cmd = 100031)
public class F30031Match extends AbstractActionHandler {

  /**
   * 游戏编码.
   */
  private static final int GAME_CODE = 30031;
  /**
   * 超时时间.
   */
  ...
  private static ScheduledExecutorService executor = new ScheduledThreadPoolExecutor(1,
      new BasicThreadFactory.Builder().namingPattern("robotUp-30031-%d").daemon(true).build());
  ...
  //      }
  //    } catch (Exception e) {
  //      LoggerUtils.error("<== 30031 [扫雷先锋.开启陪玩] ERROR: [{}]", e.getMessage());
