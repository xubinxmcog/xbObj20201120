package com.enuos.live.handle.match.f30251;
 ...
 * @version V2.0.0
 * @since 2020/8/26 12:39
 */

@Component
@AbstractAction(cmd = 100251)
public class F30251Match extends AbstractActionHandler {
    ...
    try {
      ...
      if (Objects.nonNull(result)) {
        ...
        if (Objects.nonNull(currencyInfo)) {
          ...
          if (gold >= 120) {
            if (MatchManager.MATCH_UNO_POKER.size() > 0) {
              MatchRoom matchRoom = MatchManager.MATCH_UNO_POKER.get(0);
              matchRoom.setPeopleNum(matchRoom.getPeopleNum() - 1);
              if (matchRoom.getPeopleNum() == 0) {
                MatchManager.delUnoPokerMatch(matchRoom.getRoomId());
              }
              ...
              roomRecord.setGameId(30251L).setRoomId(matchRoom.getRoomId()).setOpenWay(0).setRoomType(0)
            ...
            builder.setError("无法加入游戏.您的游戏金币不足120！").setRoomId(0);
            channel.writeAndFlush(new Packet(ActionCmd.GAME_MATCH, MatchCmd.POKER_MATCH,
                builder.build().toByteArray()));
          }
        } else {
