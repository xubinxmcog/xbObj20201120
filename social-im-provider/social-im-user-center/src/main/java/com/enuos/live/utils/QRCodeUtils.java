...
public class QRCodeUtils {
     ...
     * @param response
     */
    public static void createImage(String content, String imgPath, HttpServletResponse response) {
        try {
            BufferedImage image = createImage(content, imgPath, true);
            ImageIO.write(image, FORMAT_NAME, response.getOutputStream());
        } catch (IOException e) {
            log.error("ImageIO write error");
            e.printStackTrace();
        } catch (Exception e) {
            log.error("CreateImage error");
            e.printStackTrace();
        }
    }
    ...
    private static void insertImage(BufferedImage source, String imgPath, boolean needCompress) throws Exception {
        ...
        if (!file.exists()) {
            log.error("QRCodeUtils error with imgPath:{}", imgPath);
            return;
        }
